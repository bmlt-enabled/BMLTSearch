...
- **Domain logic extracted and tested** — time arithmetic, meeting classification, grouping, filtering, the service body tree, marker clustering, and the share payload now live framework-free in `src/lib/meetings/` under 125 unit tests
...
- **In person / online filter on Near Me and map search** — the aggregator holds ~4,300 virtual and ~950 hybrid meetings, and they were already coming back from both searches unfiltered, pinned at whatever coordinates their group registered. They can now be asked for, or excluded when only a room will do. Hybrid meetings appear under _both_ filters rather than being a third option nobody searches for
